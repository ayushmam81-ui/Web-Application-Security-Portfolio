...
* **Vulnerability Class:** Parameter-based Access Control Methods / Cookie Manipulation
...
* Cookies will be found under the storage tab—cookies tab and edit the value of admin as true.
* This step makes `wiener` as the admin—type `admin` instead of `wiener` in the URL (or navigate to `/admin`) to bypass authorization checks.
...
*Figure 1: Cookies will be found under storage tab—cookies tab and edit the value of admin as true.*

![Cookie Admin URL](../images/cookie-admin-url.png)
*Figure 2: This step makes wiener as the admin—type admin instead of wiener in the URL.*
...
1. **Server-Side Role Management:** Keep user roles and privileges strictly on the server side within a secure, validated session rather than relying on client-accessible cookies or parameters.
2. **Enforce Authorization Checks:** Validate user permissions on every incoming request to ensure clients cannot escalate privileges by altering local state variables.
